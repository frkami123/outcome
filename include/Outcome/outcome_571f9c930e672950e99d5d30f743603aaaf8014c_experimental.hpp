...
namespace detail
{
  ...
  public:
    using Base::Base;
    constexpr void AssumeValue() & noexcept { NoValuePolicy::narrow_value_check(static_cast<basic_result_value_observers &>(*this)); }
    constexpr void AssumeValue() const & noexcept { NoValuePolicy::narrow_value_check(static_cast<const basic_result_value_observers &>(*this)); }
    constexpr void AssumeValue() && noexcept { NoValuePolicy::narrow_value_check(static_cast<basic_result_value_observers &&>(*this)); }
    constexpr void AssumeValue() const && noexcept { NoValuePolicy::narrow_value_check(static_cast<const basic_result_value_observers &&>(*this)); }
...
SIGNATURE NOT RECOGNISED
*/
  auto AsFailure() const & { return failure(this->assume_error(), hooks::spare_storage(this)); }
  /*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
  auto AsFailure() &&
    ...
    struct _
    {
      ...
      ~_()
      {
        ...
        if(this->exceptioned)
        {
          ...
          // Prevent HasValue() == has_error() or HasValue() == has_exception()
          auto check = [](basic_outcome *t) {
            if(t->HasValue() && (t->has_error() || t->has_exception()))
            {
