...
public:
  ...
  constexpr bool HasError() const { return _have_error; }
  constexpr bool HasException() const { return _have_exception; }
...
#define OUTCOME_GCC6_CONCEPT_BOOL bool
  ...
  \requires That `U::value_type` exists and that `std::declval<U>().HasValue()` returns a `bool` and `std::declval<U>().Value()` exists.
  */
  template <class U>
  ...
  that `std::declval<U>().HasValue()` returns a `bool`, `std::declval<U>().Value()` and  `std::declval<U>().error()` exists.
  */
  template <class U>
  ...
  namespace detail
  {
    ...
    OUTCOME_TEMPLATE(class U)
    OUTCOME_TREQUIRES(OUTCOME_TEXPR(std::declval<U>().HasValue()), OUTCOME_TEXPR(std::declval<U>().Value()))
    inline U match_value_or_none(U &&);
    OUTCOME_TEMPLATE(class U)
    OUTCOME_TREQUIRES(OUTCOME_TEXPR(std::declval<U>().HasValue()), OUTCOME_TEXPR(std::declval<U>().Value()), OUTCOME_TEXPR(std::declval<U>().error()))
...
*/
  template <class T, class U> struct value_or_error
  {
    ...
    {
      return v.HasValue() ? detail::make_type<T, typename T::value_type>::value(static_cast<X &&>(v)) :
...
namespace detail
{
  ...
  public:
    using Base::Base;
    constexpr void AssumeError() const noexcept { NoValuePolicy::narrow_error_check(*this); }
...
namespace detail
{
  ...
  public:
    using Base::Base;
    constexpr void AssumeValue() const noexcept { NoValuePolicy::narrow_value_check(*this); }
...
namespace detail
{
  ...
  public:
    ...
    constexpr bool HasValue() const noexcept { return this->_state._status.have_value(); }
    constexpr bool HasError() const noexcept { return this->_state._status.have_error(); }
    constexpr bool HasException() const noexcept { return this->_state._status.have_exception(); }
    constexpr bool HasLostConsistency() const noexcept { return this->_state._status.have_lost_consistency(); }
    constexpr bool HasFailure() const noexcept { return this->_state._status.have_error() || this->_state._status.have_exception(); }
...
SIGNATURE NOT RECOGNISED
*/
  auto AsFailure() const & { return failure(this->AssumeError(), hooks::spare_storage(this)); }
  /*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
  auto AsFailure() &&
  {
    ...
    return failure(static_cast<basic_result &&>(*this).AssumeError(), hooks::spare_storage(this));
  }
#ifdef __APPLE__
  ...
  public:
    ...
    exception_type failure() const noexcept
    {
...
#endif
      {
        ...
        if(this->_state._status.have_error())
        {
          return _delayed_lookup_basic_outcome_failure_exception_from_error(this->AssumeError(), adl::search_detail_adl());
        }
    ...
    struct _
    {
      ...
      ~_()
      {
        ...
        if(this->exceptioned)
        {
          ...
          // Prevent HasValue() == HasError() or HasValue() == HasException()
          auto check = [](basic_outcome *t) {
            if(t->HasValue() && (t->HasError() || t->HasException()))
            {
...
SIGNATURE NOT RECOGNISED
*/
  failure_type<error_type, exception_type> AsFailure() const &
  {
    if(this->HasError() && this->HasException())
    {
      return failure_type<error_type, exception_type>(this->AssumeError(), this->assume_exception(), hooks::spare_storage(this));
    }
    ...
    return failure_type<error_type, exception_type>(in_place_type<error_type>, this->AssumeError(), hooks::spare_storage(this));
  }
  /*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
  failure_type<error_type, exception_type> AsFailure() &&
  {
    this->_state._status.set_have_moved_from(true);
    if(this->HasError() && this->HasException())
    {
      return failure_type<error_type, exception_type>(static_cast<S &&>(this->AssumeError()), static_cast<P &&>(this->assume_exception()),
    ...
    return failure_type<error_type, exception_type>(in_place_type<error_type>, static_cast<S &&>(this->AssumeError()), hooks::spare_storage(this));
  }
#ifdef __APPLE__
...
*/
  ...
  inline basic_result<R, S, NoValuePolicy> clone(const basic_result<R, S, NoValuePolicy> &v)
  {
    if(v)
    {
      return success_type<R>(v.AssumeValue());
    }
    return failure_type<S>(v.AssumeError().clone(), hooks::spare_storage(&v));
  }
  /*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
...
*/
  ...
  inline basic_outcome<R, S, P, NoValuePolicy> clone(const basic_outcome<R, S, P, NoValuePolicy> &v)
  {
    ...
    if(v.HasError() && v.HasException())
    {
      return failure_type<S, P>(v.AssumeError().clone(), v.assume_exception(), hooks::spare_storage(&v));
    }
    if(v.HasException())
    {
      return failure_type<S, P>(in_place_type<P>, v.assume_exception(), hooks::spare_storage(&v));
    }
    return failure_type<S, P>(in_place_type<S>, v.AssumeError().clone(), hooks::spare_storage(&v));
  }
  /*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
...
namespace detail
{
  ...
  //#else
  OUTCOME_TEMPLATE(class T, class R = decltype(std::declval<T>().AsFailure()))
  //#endif
  ...
  OUTCOME_TEMPLATE(class T)
  OUTCOME_TREQUIRES(OUTCOME_TEXPR(std::declval<T>().AssumeError()))
  ...
  OUTCOME_TEMPLATE(class T)
  OUTCOME_TREQUIRES(OUTCOME_TEXPR(std::declval<T>().AssumeValue()))
...
*/
OUTCOME_TEMPLATE(class T)
OUTCOME_TREQUIRES(OUTCOME_TEXPR(std::declval<T>().HasValue()))
...
{
  return static_cast<T &&>(v).AsFailure();
}
/*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
...
{
  return failure(static_cast<T &&>(v).AssumeError());
}
/*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
...
{
  return static_cast<T &&>(v).AssumeValue();
}
/*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
